...
Му уверены что если возможно обойтись только полем в одной таблице и не дублировать поля в двух таблицах нужно поступать именно так. Мы считаем что это дает более широкие возможности (хранить несколько описаний товара или несколько цен), а также не сильно уступает по скорости доступа чем запрос `description_id`. Учитывая что файл `table_name.json` открывается для чтения полностью, то по скорости при наличии достаточного количеств оперативной памяти нет особой разницы искать один `id` или несколько `price_id`.
