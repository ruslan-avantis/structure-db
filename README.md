...
## Наш подход 
Максимальная скорость доступа к данным и минимальное количество полей в самых запрашиваемых таблицах.
- Пример: одно числовое поле в таблице с 1 млн. записей может занимать более 50 Мб.

Му уверены что если возможно обойтись только полем в одной таблице и не дублировать поля в двух таблицах нужно поступать именно так. Мы считаем что это дает более широкие возможности (хранить несколько описаний товара или несколько цен), а также не сильно уступает по скорости доступа чем запрос конкретного `id`. Учитывая что файл `table_name.json` открывается для чтения полностью, то по скорости при наличии достаточного количества оперативной памяти, нет особой разницы искать один `id` или несколько `price_id`.
- Пример: Таблица product - мы убрали из нее прямую связь с таблицами: 
...
- Записи не смещаются вверх при удалении и `id`=100002 не может попасть в таблицу `table_name_1` даже если в таблице останеться 1000 записей. Мы всегда знаем в какой таблице запись с конкретным id.
