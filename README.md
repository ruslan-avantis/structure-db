...
Сообщения в тикетах и обычные сообщения сведены в одну таблицу `messages`
- `tickets` - Основная таблица тикет системы
- `tickets_status` - Статусы и связи тикетов
- `messages` - Сообщения
- `question` - Вопросы с сайта
- `chat_bot` - Автоматически отвечающий на повторяющиеся вопросы
